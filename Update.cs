...
namespace StreamCore
{
    class Update
    {
        ...
        private static readonly string LocalVersion = "2.2.7";
        private static readonly int LocalVersionNumber = 2270;
        ...
        public async static void GetPluginUpdateInfo()
        {
            ...
            try
            {
                ...
                {
                    updateMessageInfo.message = "<color=#FFFFC13C>【更新】StreamCore (bilibili edition) Plugin could update to " + PluginInfo["latest"]["Version"].Value + "!" + " What's New: " + PluginInfo["latest"]["ChangeLog"]["English"].Value + " StreamCore (bilibilil版)插件有新版本啦! 快升级到" + PluginInfo["latest"]["Version"].Value + "吧!" + "更新说明" + PluginInfo["latest"]["ChangeLog"]["ChineseSimplified"].Value + " Download at(下载地址): " + PluginInfo["latest"]["Download"].Value;
